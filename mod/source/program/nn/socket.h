...
namespace nn { namespace socket {
        ...
        s32 RecvFrom(s32 socket, void* out, ulong outLen, s32 flags, sockaddr* srcAddr, u32* srcAddrLen);
        s32 Fcntl(s32 socket, s32 cmd, s32 arg);
        
        // Errno constants (Renamed to avoid macro conflict)
        constexpr int SOC_EWOULDBLOCK = 11;
        constexpr int SOC_EAGAIN = 11;
        
        constexpr int SOC_F_SETFL = 4;
        constexpr int SOC_O_NONBLOCK = 0x800;
        constexpr int SOC_AF_INET = 2;
        constexpr int SOC_SOCK_DGRAM = 2;
        constexpr int SOC_SOCK_STREAM = 1;    // TCP
        constexpr int SOC_IPPROTO_UDP = 17;
        constexpr int SOC_IPPROTO_TCP = 6;    // TCP
        constexpr int SOC_INADDR_ANY = 0;
        constexpr int SOC_EINPROGRESS = 115;  // Connection in progress
